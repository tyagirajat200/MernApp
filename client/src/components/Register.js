...
function Register(props) {
  ...
  useEffect(()=>{
    if(auth)
      {
        console.log("auth value changed to "+auth)          //  push user to Home if he is already login
        history.push('/')
      }
      setValues({
        ...values,
  ...
  },[auth,error])                   // this effect only runs when value of auth or error changes  [component did update]

  useEffect(()=>{
        
    setValues(initialFieldValues)
      console.log("run after every rendering")                
  },[])                                           // component did mount  runs every time render
